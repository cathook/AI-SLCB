...
mark.init = function(util, math) {
   ...
   */
  mark.Line = function(from, to, color, type) {
    this.from = from;
    this.to = to;
    this.color = color;
    this.type = type;
  ...
  util.setInheritFrom(mark.Line, mark.Mark);

  mark.Line.prototype.draw = function(canvas) {
    mark._canvasBeginPath(canvas, this.color, 5, this.type);
  ...
  mark.Arrow.prototype.draw = function(canvas) {
    ...
    if (v.length() > 100) v.normalizeToThis().timesToThis(100);
  ...
  mark.Point.prototype.draw = function(canvas) {
    mark._canvasBeginPath(canvas, this.color, 3, mark.LineStyle.SOLID);
   ...
   * @var radius The radius of each dots.
   * @var colorFunc A function wich will output the color of the dot by giving
   *     it the position of that dot.
   * @var colorFunc A function wich will be called before drawing the mesh.
   */
  mark.Mesh = function(
      ltCorner, rbCorner, distance, radius, colorFunc, initFunc) {
    this.ltCorner = ltCorner;
    this.rbCorner = rbCorner;
    this.distance = distance;
    this.radius = radius;
    this.colorFunc = colorFunc;
    this.initFunc = initFunc || function() {};
  };
  ...
  mark.Mesh.prototype.draw = function(canvas) {
    ...
    for (v.x = this.ltCorner.x; v.x <= this.rbCorner.x; v.x += this.distance) {
      for (v.y = this.ltCorner.y; v.y <= this.rbCorner.y; v.y += this.distance) {
        ...
        canvas.fillRect(v.x - this.radius, v.y - this.radius,
