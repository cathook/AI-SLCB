...
math.init = function(util) {
  ...
  math.LinearFunc = function(p1, p2) {
    this._a = (p2.y - p1.y) / (p2.x - p1.x);
    this._b = p1.y - p1.x * (p2.y - p1.y) / (p2.x - p1.x);
  };

  math.LinearFunc.prototype.getY = function(x) {
    return this._a * x + this._b;
  };

  math.LinearFunc.prototype.getDerivY = function(x) {
    return this._a;
  };


  math.SegsLinearFunc = function(ps) {
    this._segs = [];
    for (var i = 0; i < ps.length - 1; ++i) {
      this._segs.push(new util.Pair(ps[i + 1].x,
                                    new math.LinearFunc(ps[i], ps[i + 1])));
    }
    this._segs[this._segs.length - 1].first = Number.POSITIVE_INFINITY;
  };

  math.SegsLinearFunc.prototype._getIndex = function(x) {
    for (var i = 0; i < this._segs.length; ++i) {
      if (x <= this._segs[i].first) {
        return i;
      }
    }
  };

  math.SegsLinearFunc.prototype.getY = function(x) {
    return this._segs[this._getIndex(x)].second.getY(x);
  };

  math.SegsLinearFunc.prototype.getDerivY = function(x) {
    return this._segs[this._getIndex(x)].second.getDerivY(x);
  };


  math.getDistanceGrad = function(v) {
